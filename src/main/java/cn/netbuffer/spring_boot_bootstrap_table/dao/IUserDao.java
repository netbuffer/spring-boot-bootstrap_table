package cn.netbuffer.spring_boot_bootstrap_table.dao;
...
//	@Query(value = "select new cn.netbuffer.sssbootstrap_table.model.Data(count(u.id),FROM_UNIXTIME(u.adddate,'%Y-%m-%d')) from User u group by FROM_UNIXTIME(u.adddate,'%Y-%m-%d')")
//	List getUserSum();
